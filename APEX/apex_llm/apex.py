...
async def call_openai_with_fallback(deployment, messages, temperature=0.1, subject=None):
    ...
    try:
        email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - Using PRIMARY OpenAI deployment ({deployment}) {subject_info}")
        ...
        email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - PRIMARY OpenAI call successful {subject_info}")
        response.client_used = "primary"
        return response
    except Exception as primary_error:
        email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - ERROR: PRIMARY OpenAI client failed: {str(primary_error)} {subject_info}")
        email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - Attempting BACKUP OpenAI deployment ({deployment}) {subject_info}")
        ...
        if backup_client is None:
            try:
                ...
                email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - Backup client initialized successfully {subject_info}")
            except Exception as backup_init_error:
                email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - ERROR: Failed to initialize backup client: {str(backup_init_error)} {subject_info}")
        ...
        try:
            ...
            email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - BACKUP OpenAI call successful {subject_info}")
            response.client_used = "backup"
            return response
        except Exception as backup_error:
            email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - ERROR: BACKUP OpenAI client also failed: {str(backup_error)} {subject_info}")
            email_log(f">> {timestamp} Script: apex.py - Function: call_openai_with_fallback - CRITICAL: Both OpenAI endpoints failed, falling back to original destination routing {subject_info}")
