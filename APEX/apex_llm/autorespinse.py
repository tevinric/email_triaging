...
from config import AZURE_STORAGE_CONNECTION_STRING, BLOB_CONTAINER_NAME, AZURE_STORAGE_PUBLIC_URL
...
async def get_template_from_blob(recipient_email):
    ...
    try:
        # Extract the mailbox name from the email address
        mailbox_name = recipient_email.split('@')[0].lower()
        
        # Template file is in the folder with the same name as the mailbox
        template_path = f"{mailbox_name}/{mailbox_name}@mail.co.za.htm"
        
        print(f">> {timestamp} Script: autoresponse.py - Function: get_template_from_blob - Retrieving template for {mailbox_name} from blob storage")
        ...
        if await blob_client.exists():
            ...
            return template_content.decode('utf-8'), mailbox_name
        else:
            # Try fallback to html extension
            alt_template_path = f"{mailbox_name}/{mailbox_name}@mail.co.za.html"
            ...
            if await blob_client.exists():
                ...
                simple_template_path = f"{mailbox_name}/{mailbox_name}.html"
                ...
                if await blob_client.exists():
                    ...
                    print(f">> {timestamp} Script: autoresponse.py - Function: get_template_from_blob - Template for {mailbox_name} not found in blob storage")
                    # Return None
                    return None, None
